...
def count_label_occurence(dataset, label, splits):
    return sum([
        len(dataset[split].filter(lambda record: record["label"] == label, num_proc=cpu_count())) for split, _ in splits.items()
    ])
...
def calculate_dataset_counts(dataset_path):
    ...
    classes = {
        name: count_label_occurence(dataset, index, splits) for index, name in enumerate(LABELS)
    }
