...
model = pipeline("text-classification", model=f"{model_path}/roberta-base-finetuned-mnli-hypothesis-only/{seed}")
...
esnli = esnli.map(add_prediction_columns, fn_kwargs={ "model": model, "seed": seed }, batched=True, batch_size=32)
