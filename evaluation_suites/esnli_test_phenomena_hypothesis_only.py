...
# model on the test split of e-SNLI separated by linguistic phenomena considering 
# only the words of premise and hypothesis that have been marked as important by human 
# annotators.
...
class Suite(evaluate.EvaluationSuite):
    def __init__(self, name):
        super().__init__(name)
        self.esnli_phenomena = DatasetDict.load_from_disk("/mnt/semproj/sem_proj22/proj_05/data/datasets/esnli_phenomena")
    ...
    def task_for(self, phenomenon: str):
        ...
        data = self.esnli_phenomena["test"].filter(lambda r: r[phenomenon] > 0, num_proc=cpu_count())
