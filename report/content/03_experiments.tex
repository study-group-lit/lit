...
This section will describe all experiments we conduct to evaluate our proposed hypotheses as well as the specific setup we use to perform the experiments including the metrics we use and the splits of our datasets.
...
% - train on filtered 2/3 and filtered 3/3
% - train longer to mitigate less data
% - evaluate predictive performance + bias
...
MCC, F1 -> Predictive performance based on the validation split of SICK
MCC because it is better for imbalanced data than F1 (citation needed)
F1 because it is popular and better for imbalanced data than Accuracy
Our data is imbalanced, as can be seen in \autoref{tab:datasets:classes}

that the models achieve is measured by testing them on the SICK dataset as it is less biased than the \ac{e-SNLI} dataset
