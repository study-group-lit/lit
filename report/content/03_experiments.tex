...
The strongly biased \acs{e-SNLI} dataset is derived from the \acs{e-SNLI} dataset in accordance with our \hyperref[par:method:mitigating_data_bias]{method} of detecting biased data. The dataset only includes samples, which are correctly predicted by the \acs{RoBERTa} model, which has been fine-tuned only on the \acs{MultiNLI} hypotheses.
